...
セグメンテーションを実装するため，まずセグメントレジスタとGDTRというレジスタを追加した．
セグメントレジスタは，複数あるセグメントの中から使用するセグメントを選択するためのレジスタで，
コード領域用のCSレジスタやデータ領域用のDSレジスタなどがある．
リアルモードであれば，
変換後のアドレス(物理アドレス)はこのセグメントレジスタの値に16をかけて変換前のアドレス(論理アドレス)を加算したものになる．
しかし，プロテクトモードにおいてはセグメントの開始アドレスやサイズ，属性が自由に設定できるようになる．

プロテクトモードにおけるセグメンテーションでは，セグメントの情報はGDTという領域に設定することになっている．
GDTはGlobal Desriptor Tableの略で，
これは１つのセグメントの開始アドレス・サイズ・属性を表すDescriptorという構造体が並んでいるメモリ領域のことで，
この領域の開始アドレスとサイズを指定するのがGDTRというレジスタになる．

エミュレータの実装では，このような，
セグメントレジスタからセグメントの番号を取得し，GDTRに設定されたGDTのメモリ領域を読み込んでアドレス変換する一連のプロセスをL2P(Logical Address to Physical Addressのつもり)という関数にまとめた．

\begin{lstlisting}
uint32_t Emulator::L2P(const x86::SRegister *sreg, const uint32_t &addr){
	if(!IsProtected()) return (sreg->reg16 * 16) + addr; // real mode
	Descriptor desc;
